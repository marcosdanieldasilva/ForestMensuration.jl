...
function _graph_table(model::TableRegressionModel{<:LinearModel})
  y = model.mf.data[1]
...
function _graph_table(models::GroupedLinearModel)
    ...
    qq = qqbuild(dist, resid)
    # Create a DataFrame combining the original data with the residuals and predicted values
    data = hcat(models.group_data, DataFrame(pred = ŷ, resid = resid), makeunique = true)
    # sort the data by rediduals values
    sort!(data, size(data, 2))
    # Insert the Q-Q plot values as new columns, ensuring unique column names
    insertcols!(data, :qx => qq.qx, :qy => qq.qy, makeunique = true)
...
- **Observed vs Fitted Values**: 
   This scatter plot compares the observed data points with the values predicted by the model. It is useful for visually assessing the accuracy of the model's predictions.

- **Residuals vs Fitted Values**: 
   This plot shows the residuals (errors) against the fitted values. Ideally, residuals should be randomly scattered around zero without any apparent pattern, indicating a good model fit.

- **Histogram of Residuals**: 
   This histogram displays the distribution of residuals. A normal distribution of residuals is one of the key assumptions of linear regression, and this plot helps verify it.

- **Normal Q-Q Plot**: 
   This plot compares the quantiles of the residuals to the quantiles of a normal distribution. It is used to check the assumption of normality in the residuals.
...
function plot_regression(model::RegressionModel)
    data = model isa TableRegressionModel ? DataFrame(model.mf.data) : model.group_data
    n = size(data, 2)
    if n > 2
    ...
    relayout!(
        ...
        width = 750, 
        height = 600,
        margin = attr(l = 40, r = 40,t = 20, b = 60),
