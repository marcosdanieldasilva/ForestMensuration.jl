@testset "stratifiedsampling" begin
  ...
  @test nrow(report.auxiliary_table) == 3
  @test report.auxiliary_table.n == [4, 3, 5]
  @test ustrip(report.auxiliary_table.x̅) ≈ [19.875, 32.8, 12.76] atol = 1e-3

  @test nrow(report.anova) == 3
  @test report.anova.DOF == [2, 9, 11]
  ...
  @testset "one stratum matches simplecasualsampling" begin
    ...
    r1 = stratifiedsampling(:stratum, :volume, 0.05, [10.0], data1).result_table
    ...
    @test ismissing(r1full.anova.F[1])
  end
